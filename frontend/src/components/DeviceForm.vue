  ...
  <div class="bg-gray-750 border border-blue-600/40 rounded-lg p-4 space-y-3">
    <div class="grid grid-cols-2 sm:grid-cols-12 gap-3">
      ...
      <div class="sm:col-span-2">
        <label class="block text-xs text-gray-400 mb-1">Min. Upload</label>
        <input
    ...
    <p class="text-xs text-gray-500">
      Identificador da máquina: <span class="font-mono text-gray-400">{{ local.machineId }}</span>
    </p>

    <div class="flex items-center gap-2 pt-1">
      <button
        @click="$emit('save', local)"
        :disabled="saving || !local.name"
...
export default {
  ...
  computed: {
    shades() {
      return deriveShades(this.local.colorHex || '#10B981');
    },
  },
};
</script>
